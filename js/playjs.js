...
let confett=document.querySelector(".winner");
confett.style.display="none";
...
function resetBoard() {
    for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
            ...
            if(currBoard.que[i*9+j]!='.')
            currCell.classList.add("block")
        }
    }
}
resetBoard();
    ...
    for (let j = 0; j < 9; j++) {
        ...
        if (currBoard.que[i*9+j]==".") {
            currCell.addEventListener("click", () => {
                ...
                currCell.classList.add("remove-cursor")
                currCell.textContent="";
                // currCell.classList.remove("remove-cursor")
            })
        }
    }
}